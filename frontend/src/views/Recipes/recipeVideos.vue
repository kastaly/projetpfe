<template>
  <div class="reel-container mx-auto h-screen items-center py-6 md:w-1/2 overflow-hidden">
    <div class="relative h-full flex">
      <!-- Display current video -->
      <div class="video-wrapper h-full flex items-center justify-center" v-for="(recipe, index) in filteredRecipes" v-show="index === recipeIndex" :key="recipe.id">
            ...
            <div class="ml-4">
              ...
              <div class="py-1">
                <a :href="`/show-recipe/${recipe.id}#recipe-section`">
                  <p class="text-gray-200">{{ recipe.steps.substring(0, 50) }}<span v-if="recipe.steps.length > 50">... <span class="text-blue-400">See more</span></span></p>
                </a>
              </div>
            </div>
          </div>
        ...
        <div class="absolute sm:left-0 flex-col items-center space-y-5 like-comment">
          <div class="like-component-wrapper flex flex-col items-center rounded-full bg-black bg-opacity-40 py-4 px-1">
            ...
            ></like-component>
            <small>{{ recipe.likes.length }}</small>
          </div>
          <div class="comments-component-wrapper flex flex-col items-center space-y-4 py-4 rounded-full bg-black bg-opacity-40">
            ...
            <small>{{ recipe.comments.length }}</small>
          </div>
        </div>
      </div>
  ...
  methods: {
    async fetchRecipes() {
      try {
        ...
        this.filteredRecipes = this.recipes.filter(recipe => recipe.video_url); // Filter recipes with video_url
    ...
    prevVideo() {
      ...
      this.recipeIndex = (this.recipeIndex - 1 + this.filteredRecipes.length) % this.filteredRecipes.length;
      this.playCurrentVideo();
    },
    nextVideo() {
      this.stopCurrentVideo();
      this.recipeIndex = (this.recipeIndex + 1) % this.filteredRecipes.length;
      this.playCurrentVideo();
    },
    toggleMute() {
